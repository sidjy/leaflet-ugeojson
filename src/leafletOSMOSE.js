L.OsmoseLayer = L.GeoJSON.extend({
    options: {
      debug: false,
      light: true,
      endpoint: "http://osmose.openstreetmap.fr/fr/api/0.2/",
      parameters: {},
      maxRequests: 5,
      pollTime:0,
      once: false,
  ...
  onMoveEnd: function () {
    ...
    params="&bbox="+bounds.toBBoxString()+"&zoom="+this._map.getZoom();
    if (this.options.debug) {
...
L.OsmoseLayer = function (uOptions, options) {
  return new L.OsmoseLayer(uOptions, options);
};
